...
// static cx_sha3_t key_sha, digest_sha;
// static cx_sha3_t sha;

static cx_sha3_t sha1, sha2;

static unsigned char bytes[NUM_HASH_BYTES * 2];

// -------------------------------------------

cx_sha3_t *get_sha(unsigned index)
{
  if(index == 0) return &sha1;
  return &sha2;
//  return malloc(sizeof(cx_sha3_t));
}
                     ...
                     unsigned int security, unsigned char *address_bytes)
{
    ...
    //cx_sha3_t key_sha, digest_sha;w

    cx_sha3_t *key_sha = &sha1;
    cx_sha3_t *digest_sha = &sha2;
